...
Edited by Bill Levay for GLBT Historical Society to avoid strings with content=""
-->
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template match="*:div[@class='ocr_carea']">
      ...
      <xsl:choose>
        <xsl:when test="not(*:p[@class='ocr_par'] = '')">
  ...
  <xsl:template match="*:p[@class='ocr_par']">
      <xsl:variable name="box" select="tokenize(mf:getBox(@title), ' ')"/>
      <xsl:choose>
        <xsl:when test="not(*:span[@class='ocr_line'] = '')">
  ...
  <xsl:template match="*:span[@class='ocr_line']">
      <xsl:variable name="box" select="tokenize(mf:getBox(@title), ' ')"/>
        <xsl:choose>
          <xsl:when test="not(*:span[@class='ocrx_word'] = '')">
